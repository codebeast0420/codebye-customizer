...
const Header = ({ showContactModal, message }) => {
    ...
    if (!product.data.id || message == "") {
      ...
      if (product.data.id == 186 || product.data.id == 185) {
        ...
        if (product.data.id == 408) {
          q = query(collection(firebaseDB, 'aquafiore-earrings'))
        }
        if (product.data.id == 402) {
          q = query(collection(firebaseDB, 'aquafiore-bracelet'))
        }
        if (product.data.id == 405) {
          q = query(collection(firebaseDB, 'aquafiore-pendant'))
        }
        onSnapshot(q, async (querySnapshot) => {
          console.log('query', querySnapshot.docs);
          priceDB = querySnapshot.docs.filter(doc => (
  ...
  const buy = async () => {
    ...
    DomToImage.toJpeg(image, { quality: 0.95 }).then(async (dataUrl) => {
      ...
      axios.post("https://codeby-backend-tau.vercel.app/add-to-cart", { productId: productId }).then((res) => {
        ...
        if (variant == "not_found") {
          // create one
          axios
            .post("https://codeby-backend-tau.vercel.app/create-cart", {
              productId: productId,
              price: price,
            })
