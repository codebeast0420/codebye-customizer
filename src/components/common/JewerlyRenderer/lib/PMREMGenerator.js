...
THREE.PMREMGenerator = function ( sourceTexture, samplesPerLevel, resolution ) {
	...
	this.cubeTexture.useManualMipmaps = true;
	
	const numMips = Math.log2(this.cubeLods[0].width) + 1;
	for(let i = 0; i < 6; i ++) {
		let res = this.resolution;
		const texture = new THREE.DataTexture();
		this.cubeTexture.image[i] = texture;
		for( let m = 0; m < numMips; m++ ) {
			const bufferData = new Uint8Array(4 * res * res);
			texture.mipmaps[m] = { data:bufferData, width:res, height:res };
			res /= 2;
		}
	}
	this.cubeTexture.format = this.sourceTexture.format;
	...
	this.cubeTexture.anisotropy = this.sourceTexture.anisotropy;
	this.cubeTexture.encoding = this.sourceTexture.encoding;
};
...
THREE.PMREMGenerator.prototype = {
	...
	readCubeTextureMipMap ( renderer ) {

		for(let i = 0; i < 6; i ++) {
			const texture = this.cubeTexture.image[i];
			const numMips = Math.log2(this.cubeLods[0].width) + 1;
			for(let m = 0; m < numMips; ++m ) {
				const renderTarget = m < this.numLods ? this.cubeLods[m] : this.cubeLods[this.numLods-1];
				renderTarget.activeCubeFace = i;
				renderer.setRenderTarget( renderTarget );
				const buffer = texture.mipmaps[m].data;
				if(m < this.numLods) {
