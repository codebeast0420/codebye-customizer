...
import { getProductAction, getProductsAction, getProductPartsAction, getThemesAction } from "../../store/actions";
...
class Pendant extends React.Component {
	...
	constructor(props) {
		...
		if (product.data.id) {
			...
			Promise.all([
				dispatchGetProduct(405, { message: "a" }),
				dispatchGetProductParts(405),
				dispatchGetThemes(),
			]).then((result) => {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		console.log('prev configuration', prevProps.configuration);
	...
	onChangeMsg = (e) => {
		...
		if (product.data.id !== 2096 && product.data.id !== 2124) {
			...
			if (product.data.id !== 186 && product.data.id !== 185) {
				console.log('no amanti', product.data.id);
				console.log('405');
...
export default connect(mapStateToProps, mapDispatchToProps)(Pendant);
