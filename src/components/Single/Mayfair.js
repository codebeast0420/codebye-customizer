...
class Mayfair extends React.Component {
	state = {};
	...
	constructor(props) {
		...
		if (product.data.id) {
			...
			Promise.all([
				dispatchGetProduct(185, { message: "a" }),
				dispatchGetProductParts(185),
				dispatchGetThemes(),
			]).then((result) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(Mayfair);
